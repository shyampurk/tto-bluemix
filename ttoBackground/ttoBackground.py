...
import B_ne_testprep
import B_bd_testprep
import B_sf_testprep
import B_ne_scikit
import B_bd_scikit
import B_sf_scikit
...
def function_routenewarkedison():
	...
	while True:
		...
		if (ne_calc_minute == 0):
			...
			ne_df,netestprep_return = B_ne_testprep.nealgo(newarkedison_loctime,induceTime,induceWeather,induceTemparature)
					
			if (netestprep_return == True):
				nescikit_return = B_ne_scikit.ne_scikitalgo(ne_df)
			else:
				pass
...
def function_routebrooklyndenville():
	...
	while True:
		...
		if (bd_calc_minute == 0):
			...
			bd_df,bdtestprep_return = B_bd_testprep.bdalgo(brooklyndenville_loctime,induceTime,induceWeather,induceTemparature)
			
			if (bdtestprep_return == True):
				bdscikit_return = B_bd_scikit.bd_scikitalgo(bd_df)
			else:
				pass
...
def function_routesanfrancisco():
	...
	while True:
		...
		if (sf_calc_minute == 0):
			...
			sf_df,sftestprep_return = B_sf_testprep.sfalgo(sanfrancisco_loctime,induceTime,induceWeather,induceTemparature)
			
			if (sftestprep_return == True):				
				sfscikit_return= B_sf_scikit.sf_scikitalgo(sf_df)	
			else:
				pass				
